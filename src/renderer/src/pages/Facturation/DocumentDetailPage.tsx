...
export default function DocumentDetailPage({ type }: { type: DocumentType }): React.JSX.Element {
  ...
  if (!doc || !profile || !client) {
    return (
      <div className="p-8">
        <p className="text-sm text-slate-500">Document introuvable.</p>
  ...
  return (
    <div>
      ...
      <div className="print:hidden p-8 max-w-3xl mx-auto">
        <Link to={`/facturation/${basePath}`} className="text-sm text-slate-500 hover:text-brand-600">
          ← Retour à la liste
        </Link>
        ...
        <div className="mt-6 bg-white rounded-xl border border-slate-200 p-4">
          <h3 className="text-xs font-medium text-slate-500 mb-2">Aperçu</h3>
      ...
      <div className="print:p-0 px-8 pb-8 max-w-3xl mx-auto print:max-w-none">
        <div className="bg-white border border-slate-200 print:border-0 rounded-xl print:rounded-none p-10 print:p-0 text-sm text-slate-800">
          <div className="flex justify-between items-start">
            <div>
              ...
              {profile.siret && <p className="mt-1 text-slate-500">SIRET : {profile.siret}</p>}
              {profile.tvaIntracom && <p className="text-slate-500">TVA intracom. : {profile.tvaIntracom}</p>}
              {profile.email && <p className="text-slate-500">{profile.email}</p>}
              {profile.telephone && <p className="text-slate-500">{profile.telephone}</p>}
            </div>
            <div className="text-right">
              <h1 className="text-xl font-bold uppercase tracking-wide">
                {type === 'devis' ? 'Devis' : 'Facture'}
              </h1>
              <p className="text-slate-500 mt-1">N° {doc.numero}</p>
              <p className="text-slate-500">Émis le {doc.dateEmission}</p>
              {doc.dateEcheance && (
          ...
          <div className="mt-8 flex justify-end">
            <div className="text-right">
              <p className="text-xs uppercase text-slate-400 mb-1">Client</p>
          ...
          {project && <p className="mt-4 text-xs text-slate-400">Projet : {project.nom}</p>}

          <table className="w-full mt-8 text-sm border-t border-slate-300">
            <thead>
          ...
          <div className="mt-4 flex justify-end">
            <div className="w-64 space-y-1">
              ...
              {franchiseEnBase ? (
                <div className="text-xs text-slate-500">TVA non applicable, art. 293 B du CGI</div>
              ) : (
          ...
          {doc.conditionsPaiement && (
            <div className="mt-6">
              <p className="text-xs uppercase text-slate-400 mb-1">Conditions de paiement</p>
          ...
          {doc.notes && (
            <div className="mt-4">
              <p className="text-xs uppercase text-slate-400 mb-1">Notes</p>
              <p className="whitespace-pre-wrap">{doc.notes}</p>
            </div>
          )}

          <div className="mt-10 pt-4 border-t border-slate-200 text-xs text-slate-400 whitespace-pre-wrap">
