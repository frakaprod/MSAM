...
export default function SupplierInvoicesListPage(): React.JSX.Element {
  ...
  // Regroupées par mois de facture, comme les factures/devis émis et le
  // module Fiscalité.
  const groups = useMemo(() => {
    const map = new Map<string, SupplierInvoice[]>()
    for (const invoice of invoices) {
      const key = invoice.dateFacture.slice(0, 7)
    ...
    <div className="p-8 pt-6 max-w-5xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {groups.map(([monthKey, monthInvoices]) => {
            const monthTotal = monthInvoices.reduce((sum, i) => sum + i.montantTTC, 0)
            return (
              <div
                key={monthKey}
